...
- 지역변수를 변경해도 렌더링이 트리거 되지않는다. 리엑트는 새 데이터로 컴포넌트를 다시 렌더링해야 한다는 것을 인식하지 못한다.
  <br />
...
- 변수를 업데이트 하고 이랙트를 트리거하여 컴포넌트를 다시 렌더링하는 상태 설정 함수(state setter function)이다.
...
- 내부적으로 리액트는 모든 구성요소에 대한 상태 페어 배열을 보유한다. 또한 렌더링 전에 설정된 현재 페어의 인덱스를 유지한다.
